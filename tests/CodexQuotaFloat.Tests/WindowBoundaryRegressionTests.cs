...
public sealed class WindowBoundaryRegressionTests
{
    ...
    [Fact] public void ExpandedClampUsesExpandedHeight() => Assert.Equal(778, WindowPositionService.Clamp(new(0, 1000), new(340, FloatingWindowLayoutMetrics.MinimumExpandedHeight), Area).Y);
    [Fact] public void ExpandedClampUsesFullWidth() => Assert.Equal(1580, WindowPositionService.Clamp(new(2000, 0), new(340, FloatingWindowLayoutMetrics.MinimumExpandedHeight), Area).X);
    [Fact] public void ExpandedBottomRightPointKeepsWindowVisible() { var p = WindowPositionService.Clamp(new(1900, 1000), new(340, FloatingWindowLayoutMetrics.MinimumExpandedHeight), Area); Assert.True(p.X + 340 <= Area.Right && p.Y + FloatingWindowLayoutMetrics.MinimumExpandedHeight <= Area.Bottom); }
    ...
    [Fact] public void NaNActualHeightFallsBackToExpandedHeight() => Assert.Equal(262, WindowPositionService.GetEffectiveSize(340, 340, double.NaN, 0, true).Height);
    ...
    [Fact] public void HugeWindowUsesFiniteFallbackSize() { var size = WindowPositionService.GetEffectiveSize(99999, 99999, 99999, 99999, true); Assert.Equal(new Size(340, 262), size); }
}
