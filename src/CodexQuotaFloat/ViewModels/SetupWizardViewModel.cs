...
public sealed class SetupWizardViewModel : INotifyPropertyChanged
{
    ...
    private SetupCheckResult _result = new(SetupStatus.ConnectionFailed);
    ...
    public string Title => Result.Status switch { SetupStatus.CodexNotFound => "需要安装 Codex CLI", SetupStatus.NotLoggedIn => "需要登录 Codex", SetupStatus.ApiKeyMode => "当前使用 API Key 模式", SetupStatus.VersionTooOld => "需要更新 Codex CLI", _ => "Codex CLI 连接状态" };
    ...
    public void SetResult(SetupCheckResult result) => Result = result;
